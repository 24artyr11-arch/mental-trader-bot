...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    keyboard = [[
        InlineKeyboardButton("GET BOT", callback_data="get_bot")
    ]]
...
async def choose_plan(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # The chosen plan is encoded directly into the next buttons.
    # This is safer than relying only on in-memory user_data:
    # even after a bot restart, an already-open payment menu still works correctly.
    keyboard = [
        [
...
async def payment(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # First message: payment amount.
    await query.edit_message_text(
        f"Pay the amount: <b>{amount_text}</b>",
        parse_mode="HTML",
    )

    # Separate final message: wallet address.
    # The bot flow intentionally ends here.
...
def main() -> None:
    if not BOT_TOKEN:
        raise RuntimeError(
            "BOT_TOKEN is not set. Add it to Bothost environment variables or to a local .env file."
        )
